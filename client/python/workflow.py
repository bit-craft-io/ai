...
def query_agent(user_text: str | None):
    ...
    outputs = result.get("data", {}).get("outputs") if isinstance(result, dict) else None

    if outputs:
        message = outputs.get("message") if isinstance(outputs, dict) else outputs

        print("\n" + "=" * 60)
        print(f"[Response]: {message}")
        print("=" * 60 + "\n")
    else:
        print("[-] Error: 'outputs' field missing in response.")
        print(result)
